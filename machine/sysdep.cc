...
//	Abort on error.
//----------------------------------------------------------------------
bool SendToSocket(int sockID, char *buffer, int packetSize, char *toName) {
  struct sockaddr_un uName;
  int retVal;
  ...
  //If retVal doesn't match packetSize, but is not -1, then
  //you probably didn't set the size properly for your packet
  //The ASSERT is to alert you to this
  ASSERT(retVal == packetSize);
